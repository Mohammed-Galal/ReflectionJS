...
export function useState(initState) {
  ...
  const targetedComponent = Components.currentContext,
    proxyFN = targetedComponent.proxyFN,
  ...
  return [
    ...
    function (newVal) {
      ...
      if (!hooks.useBatchOn) return proxyFN();
      targetedComponent.useBatch.add(proxyFN);
    },
  ];
}
...
//   const targetedComponent = Components.currentContext;
//   !Boolean(serverCollate[state]) && (serverCollate[state] = new Set());
//   serverCollate[state].add(targetedComponent.proxyFN);

//   return [
//     demoServerState[state],
//     function (fn) {
//       demoServerState[state] = fn(demoServerState[state]);

//       if (hooks.useBatchOn) {
//         serverCollate[state].forEach(function ($) {
//           targetedComponent.useBatch.add($);
//         });
//         serverCollate[state].clear();
//       } else
//         serverCollate[state].forEach(function ($) {
//           $();
//         });
